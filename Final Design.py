...
# This code forces the gap below top flange to be 75-20 mm, because additional area was used.
...
side_boards_key_changes = {
    0: (175, board_thickness, 75),
    L: (175, board_thickness, 75)
}
"""
...
"""
length_of_addition = 350
flaps_key_changes = {
    0: (board_thickness, 10),
    600-length_of_addition/2-1: (board_thickness, 10),
    600-length_of_addition/2: (board_thickness, 10+12),
    600+length_of_addition/2: (board_thickness, 10+12),
    600+length_of_addition/2+1: (board_thickness, 10),
    L: (board_thickness, 10)
}
"""
...
"""
areas = top_thickness * top_width + top_additional_layers*board_thickness*(side_gap-20) + 2 * side_thickness * side_height + bottom_thickness * bottom_width + 2 * flap_thickness * flap_length
...
y_bars = (top_thickness * top_width * (side_height + top_thickness/2)
          + top_additional_layers*board_thickness * (side_gap-20) * (side_height-top_additional_layers*board_thickness/2)
...
Is = top_width * top_thickness**3 / 12 + top_thickness * top_width * (side_height + top_thickness/2 - y_bars)**2 \
        + (side_gap-20) * (top_additional_layers*board_thickness)**3 / 12 + top_additional_layers*board_thickness * (side_gap-20) * (side_height-top_additional_layers*board_thickness/2 - y_bars)**2 \
...
Q_centres = 2 * y_bars * side_thickness * y_bars/2 \
            ...
            + np.array([max(0, y_bars[x] - (side_height[x] - top_additional_layers[x]*board_thickness))**2 / 2 * (side_gap[x]-20) for x in x_coords])  # squared / 2 because it's h * h/2
...
for x in x_coords:
    if top_additional_layers[x] > 1:  # one layer shares Q with glue flaps
        print("THIS HAPPENED, PLEASE CHECK")
        ...
        additional_layers_largest_Q[x] = 2 * y_for_Q * side_thickness[x] * (y_bars[x] - y_for_Q/2) \
                                         ...
                                         + (top_additional_layers[x] - num_additional_boards_closest_to_y_bar + 1) * board_thickness * (side_gap[x]-20) * (y_bars[x] - (y_for_Q-(top_additional_layers[x] - num_additional_boards_closest_to_y_bar + 1) * board_thickness/2))
...
centroid_axis_shear_fail_Vs = tau_shear * Is * (2*side_thickness
                                                + np.array([(side_gap[x]-20) if y_bars[x] > (side_height[x] - top_additional_layers[x]*board_thickness) else 0 for x in x_coords])  # If centre board extends there, add it
                                                ) / Q_centres
print("Centroid axis shear fail V", [i for i in centroid_axis_shear_fail_Vs])
# Glue shear (tau = VQ/Ib -> V = tau*Ib/Q)
glue_shear_fail_Vs = glue_shear_strength * Is * (2*flap_length+2*side_thickness + (np.array([(side_gap[x]-20) if top_additional_layers[x] > 0 else 0 for x in x_coords]))) / Q_glues
print("Glue shear fail V", [i for i in glue_shear_fail_Vs])
# layer shear (tau = VQ/Ib -> V = tau*Ib/Q)
top_additional_layers_glue_fail_Vs = glue_shear_strength * Is * (2*side_thickness + np.array([(side_gap[x]-20) if top_additional_layers[x] > 0 else 999999 for x in x_coords])) / additional_layers_largest_Q  # additional_layers_largest_Q is TINY if no additional layers, b is huge if no additional layers -> to ensure it's not used as a fail case
...
webs_shear_buckling_SF = webs_shear_buckling_stress * Is * (2*side_thickness + np.array([(side_gap[x]-20) if y_bars[x] > (side_height[x] - top_additional_layers[x]*board_thickness) else 0 for x in x_coords])) / Q_centres
print("webs shear buckling SF", [x for x in webs_shear_buckling_SF])
...
plt.suptitle("Final Bridge Design Failure Graphs")
plt.savefig("Final Iteration.png")
plt.show()
